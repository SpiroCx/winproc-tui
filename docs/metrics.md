...
| `Compat` | Not recorded | Indicates elevated-compat behavior from `__COMPAT_LAYER`, populated automatically by background Environment collection while this column is visible. | Process Info `Environment` worker (`NtQueryInformationProcess` + remote environment block parse) | `Admin` when the value contains `Admin`, otherwise `--` |
