...
| `Compat` | Not recorded | Indicates elevated-compat behavior from `__COMPAT_LAYER` when Process Info `Environment` has collected data for that process identity. | Process Info `Environment` worker (`NtQueryInformationProcess` + remote environment block parse) | `Admin` when the value contains `Admin`, otherwise `--` |
