...
Find processes by file searches disk-file handles across the host, independently of the Processes filter, selection, and Tracked-only setting. Opening the browser or editing the query performs no scan. Only an explicit search or repeat starts collection. Live, display pause, and Recording allow this investigation; Log view does not.
...
First opening the Network workspace or first activating the tab requests one capture. Returning to a hidden workspace preserves its capture, filter, and selection without collecting again. Tools groups Network endpoints and Find processes by file, with direct keyboard routes to both workspaces and back to Processes. Opening file search does not start a scan. Network uses the main body height, with a neighboring endpoint/owner summary when there is enough width to retain readable address columns. At narrower widths the full-width table and explicit details view preserve access to complete values. Refresh is explicit, stays on the independent Network worker, and does not queue another request for the same pending session. The worker has a bounded request queue. Tab switches retain the capture, filter, and selection; ending a session invalidates its pending work; hiding a workspace allows an already requested capture to finish, but never opens a late owner inspector over another view. Every result must match its target, generation, and request ID. Selection survives refresh by endpoint key where possible.

The four protocol/family tables are captured separately, so the report describes a capture interval rather than an atomic system snapshot. Successful tables remain visible if another table fails. The view distinguishes displayed endpoint rows from all captured endpoint rows, alongside capture time and successful-table coverage. Owner-resolution warnings count endpoint rows with unavailable or unverified ownership in both the displayed subset and the whole capture; they do not count distinct processes. Partial-table coverage and ownership warnings can appear together. Full capture errors are available in details, including when no endpoint is selected. A failed refresh preserves the previous capture with its original timestamp and a visible notice.
