...
- **ネットワークの調査**: ヘッダーの **Network** または **NW/DISK** からシステム全体の TCP/UDP のアドレス、ポート、所有プロセスを確認し、Process Info に移動できます。ビューを切り替えても一覧とフィルターを保持します。一覧は操作時に取得し、レコーディングには含めません。
...
- ヘッダーの **Find file users** からファイル名やパスで使用中のプロセスを検索し、そのプロセスの Files タブで詳しく調べる。検索できなかった範囲も確認できます。
